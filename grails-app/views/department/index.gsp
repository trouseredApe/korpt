		...
		<div id="list-department" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="contactId" title="${message(code: 'department.contactId.label', default: 'Contact Id')}" />
					
						<g:sortableColumn property="addressId" title="${message(code: 'department.addressId.label', default: 'Address Id')}" />
				...
				<g:each in="${departmentInstanceList}" status="i" var="departmentInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${departmentInstance.id}">${fieldValue(bean: departmentInstance, field: "contactId")}</g:link></td>
					
						<td>${fieldValue(bean: departmentInstance, field: "addressId")}</td>
